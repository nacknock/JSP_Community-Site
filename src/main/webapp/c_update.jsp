...
Board board = new boardDAO().getBoard_comm(b_num,c_num,animal);
...
if (b_num!=(board.getB_num())) {
	...
	script.println("alert('권한이 없습니다.case-a')");
...
if (c_num!=(board.getC_num())) {

	PrintWriter script = response.getWriter();
	
	script.println("<script>");
	
	script.println("alert('권한이 없습니다.case-b')");
	...
	if (request.getParameter("b_content") == null || request.getParameter("b_content").equals("") ) {
		...
		int result = boardDAO.update_comm(b_num, c_num, animal, request.getParameter("b_content"));
