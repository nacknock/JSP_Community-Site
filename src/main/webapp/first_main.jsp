...
%>
	...
	<div class="row tables" id="tables">
		<div class="tableset" id="tableset1">
			...
			<section>
			<li class="list list-title">강아지</li>
	<%
    boardDAO boardDAO = new boardDAO();
	ArrayList<Board> list2 = boardDAO.getList2("doggy");
	if(list2.size()<9){
		for (int i = 0; i < list2.size(); i++) {
	%>
			<li class="list"><a href="view.jsp?b_num=<%=list2.get(i).getB_num()%>&&animal=doggy"><%=list2.get(i).getB_title()%></a></li>
	<%
		}
	}else{
	for (int i = 0; i < 10; i++) {
	...
	%>
		...
		<div class="tableset"  >
			<section>
			<li class="list list-title">고양이</li>
	<%
	ArrayList<Board> list3 = boardDAO.getList3("cat");
	if(list3.size()<9){
		for (int i = 0; i < list3.size(); i++) {
	%>
			<li class="list"><a href="view.jsp?b_num=<%=list3.get(i).getB_num()%>&&animal=cat"><%=list3.get(i).getB_title()%></a></li>
	<%
		}
	}else{
		for (int i = 0; i < 10; i++) {
	...
	%>
		...
		<div class="tableset">
			<section>
			<li class="list list-title">그 외</li>
	<%
	ArrayList<Board> list4 = boardDAO.getList4("etc");
	if(list4.size()<9){
		for (int i = 0; i < list4.size(); i++) {
	%>
			<li class="list"><a href="view.jsp?b_num=<%=list4.get(i).getB_num()%>&&animal=etc"><%=list4.get(i).getB_title()%></a></li>
	<%
		}
	}else{
	for (int i = 0; i < 10; i++) {
